<div class="search">
	<form>
		<input type="search" id="user-search" placeholder="Search">
	</form>
	<div class="search-suggestion">
			<a href="/wuwana">
				<li>Wuwana</li>
			</a>
			<hr>
			<a href="/wuwana">
				<li>Camden Coffee Roaster</li>
			</a>
			<hr>
			<a href="/wuwana">
				<li>Right Side Coffee</li>
			</a>
			<hr>
			<a href="/wuwana">
				<li>Coffee</li>
			</a>
	</div>
	<div id="search-btn">
		<svg focusable="false" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path fill-rule="evenodd" clip-rule="evenodd" d="M11 2C15.9706 2 20 6.02944 20 11C20 13.1248 19.2637 15.0776 18.0323 16.6172L21.7071 20.2929C22.0976 20.6834 22.0976 21.3166 21.7071 21.7071C21.3466 22.0676 20.7794 22.0953 20.3871 21.7903L20.2929 21.7071L16.6172 18.0323C15.0776 19.2637 13.1248 20 11 20C6.02944 20 2 15.9706 2 11C2 6.02944 6.02944 2 11 2ZM11 4C7.13401 4 4 7.13401 4 11C4 14.866 7.13401 18 11 18C14.866 18 18 14.866 18 11C18 7.13401 14.866 4 11 4Z" fill="#8B8989"/>
		</svg>
	</div>
</div>